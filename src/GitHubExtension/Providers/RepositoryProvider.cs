    ...
    IAsyncOperation<RepositoriesResult> IRepositoryProvider.GetRepositoriesAsync(IDeveloperId developerId)
    {
        return Task.Run(async () =>
        {
            ...
            catch (Exception ex)
            {
                ...
                _log.Error(ex, "Failed getting list of repositories.");
    ...
    public IAsyncOperation<RepositoryResult> GetRepositoryFromUriAsync(Uri uri, IDeveloperId? developerId)
    {
        return Task.Run(() =>
        {
            ...
            catch (AggregateException e)
            {
                ...
                {
                    _log.Error($"Can't find {owner}/{repoName}");
                ...
                {
                    _log.Error($"Forbidden access to {owner}/{repoName}");
    ...
    public IAsyncOperation<ProviderOperationResult> CloneRepositoryAsync(IRepository repository, string cloneDestination, IDeveloperId? developerId)
    {
        return Task.Run(() =>
        {
            ...
            if (developerId != null)
            {
                ...
                {
                    _log.Error(e, "Could not get credentials.");
            ...
            {
                _log.Error(recurseException, "Could not clone all submodules.");
            ...
            {
                _log.Error(nameConflictException, "Name conflict");
            ...
            {
                _log.Error(libGitTwoException, $"Either no logged in account has access to this repository, or the repository can't be found.");
